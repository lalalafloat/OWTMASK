...
def mask_nms(seg_masks, scores, category_ids, nms_thr=0.5):
    ...
    # print('mask.min = {}, mask.max = {}'.format(seg_masks.min(), seg_masks.max()))
    # seg_masks = seg_masks.sigmoid()>0.5
    seg_masks = seg_masks>0.5  # [TODO]  这里mask rcnn不要过sigmoid
...
class IDOL_Tracker(object):
    ...
    def __init__(self,
        ...
        self.match_score_thr = match_score_thr  # [ATTN]
        ...
        assert match_metric in ['bisoftmax', 'softmax', 'cosine', 'mix']
    ...
    def match(self, bboxes, labels, masks, track_feats, frame_id,indices):
        ...
        # print('sum_valids = {}'.format(sum(valids)))  # [ATTN] OWTMASK vid4这里居然只有20+ 30+

        valids = ops.nms(bboxes[:, :4], bboxes[:, -1], self.nms_thr_pre)  # self.nms_thr_pre = 0.5
        # print('len_valids_boxnms = {}'.format(valids.shape))  # [ATTN] 挺重要的输出
        # print('first_mask_nms cost = {}'.format(time.time() - t_s))  # 原图2s, 原本缩放的是1e-5级别
        
        # print('embeds_pre.shape = {}, valid.long().sum() = {}'.format(embeds.shape, sum([int(_) for _ in valids])))
        # embeds_pre.shape = torch.Size([63, 256]), valid.long().sum() = 1
        ...
        if bboxes.size(0) > 0 and not self.empty:
            ...
            if self.match_metric == 'longrang':
                ...
                # print('embeds.shape = {}, memo_embeds.shape = {}'.format(embeds.shape, memo_embeds.shape))
                # embeds.shape = torch.Size([1, 256]), memo_embeds.shape = torch.Size([1, 256])
            ...
            elif self.match_metric == 'mix':
                feats = torch.mm(embeds, memo_embeds.t())
                d2t_scores = feats.softmax(dim=1)   
                t2d_scores = feats.softmax(dim=0)
                scores = (d2t_scores + t2d_scores) / 2
                # print('bboxes.shape = {}, memo_bboxes.shape = {}'.format(bboxes.shape, memo_bboxes.shape))
                ious = ops.box_iou(bboxes[:, :4], memo_bboxes[:, :4])
                # print('feats.shape = {}, ious.shape = {}'.format(feats.shape, ious.shape))
                scores = (scores + ious) / 2
            else:
                raise NotImplementedError
            for i in range(bboxes.size(0)):
                
                if self.frame_weight:  # True
                    ...
                    conf, memo_ind = torch.max(scores[i, :], dim=0)
                id = memo_ids[memo_ind]  # [ATTN]
                # print('scores.shape = {}, conf = {}'.format(scores.shape, conf))
                # scores.shape = torch.Size([1, 1]), conf = 1.0
                # print('(conf > self.match_score_thr).long().sum() = {}, self.match_score_thr = {}'.format((conf > self.match_score_thr).long().sum(), self.match_score_thr))  # 这个只有1 难怪外面是1
                if conf > self.match_score_thr: 
                    if id > -1:  
                        ids[i] = id
                        scores[:i, memo_ind] = 0
                        scores[i + 1:, memo_ind] = 0
            # self.addnew_score_thr = 0.0
            new_inds = (ids == -2) & (bboxes[:, 4] > self.addnew_score_thr).cpu()  # [ATTN] 这里boxes[:4]>0.2还过滤了一波
            ...
            # print(bboxes.shape)  # [num_box, 5]
            # print(masks.shape)  # torch.Size([69, 1, 1200, 1920])
            mask_ious = ops.box_iou(bboxes[:, :4][unselected_inds], bboxes[:, :4])
            for i, ind in enumerate(unselected_inds):
                if (mask_ious[i, :ind] < self.nms_thr_post).all():  # [ATTN] 不太清楚这一步在干嘛
                    ids[ind] = -1 
